...
export module ProcessDefinition {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { ERC165_ID_VERSIONED_ARTIFACT(call = defaultCall): Promise<[
                ...
                subProcessModelId: Buffer;
                subProcessDefinitionId: Buffer;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["8535517F68417FECB5815FC8EE689D6176B52551602CD478F4869FA464105D4E"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogActivityDefinitionCreation()); }, start, end); }, LogDataMappingCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                processDefinitionAddress: string;
                activityId: Buffer;
                dataMappingId: Buffer;
                ...
                dataStorage: string;
                direction: number;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["8F625BB1B0890F994D35093A4593A61E7D37802F8F6E6D454C6826FBD517067E"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogDataMappingCreation()); }, start, end); }, LogIntermediateEventDefinitionCreation(callback: (err?: Error, event?: {
                eventURN: Buffer;
                processDefinitionAddress: string;
                eventId: Buffer;
                ...
                timestampConstant: number;
                durationConstant: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["FFCC9917FD665BC2A9845E37C738003462EFDB9CE57700A86AAE2ACD73ACE689"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogIntermediateEventDefinitionCreation()); }, start, end); }, LogProcessDefinitionCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                processDefinitionAddress: string;
                id: Buffer;
                interfaceId: Buffer;
                modelId: Buffer;
                modelAddress: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["38D94ABEB96A84AB50E93C24191394BF293459AED56A464639DFA03ACDABA445"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogProcessDefinitionCreation()); }, start, end); }, LogProcessDefinitionInterfaceIdUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                processDefinitionAddress: string;
                interfaceId: Buffer;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["71E1CAF18038EC62CCE99F19463F24D1083D26DD1FCC8A956A1821C9F07FBC1C"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogProcessDefinitionInterfaceIdUpdate()); }, start, end); } } as const, listenerFor: <T extends EventName>(eventNames: T[]): TypedListener<T> => listenerFor(client, address, events, decode, eventNames) as unknown as TypedListener<T>, listener: listenerFor(client, address, events, decode, Object.keys(events) as EventName[]) as TypedListener<EventName> } as const);
