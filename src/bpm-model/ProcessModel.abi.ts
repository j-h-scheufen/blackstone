...
export module ProcessModel {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { ERC165_ID_VERSIONED_ARTIFACT(call = defaultCall): Promise<[
            ...
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["2DA2152DF872B20ECBAF3B6BB469E4212093286B55955B935B270357AD91E001"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogProcessModelCreation()); }, start, end); }, LogProcessModelDataCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                dataId: Buffer;
                dataPath: Buffer;
                modelAddress: string;
                parameterType: number;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["1C8C663AB878CA9FDC7DB26A46A7DCF655CA7BBA1E465D375B58CAD7F9ACB16D"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogProcessModelDataCreation()); }, start, end); }, LogProcessModelFileReferenceUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                modelAddress: string;
                modelFileReference: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["8EDBE78B5566AA795A0BF1299D6FB25E7137F2ACA9F74A3E15A27D50AFCE33F5"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogProcessModelFileReferenceUpdate()); }, start, end); } } as const, listenerFor: <T extends EventName>(eventNames: T[]): TypedListener<T> => listenerFor(client, address, events, decode, eventNames) as unknown as TypedListener<T>, listener: listenerFor(client, address, events, decode, Object.keys(events) as EventName[]) as TypedListener<EventName> } as const);
