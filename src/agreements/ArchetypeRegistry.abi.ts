...
export module ArchetypeRegistry {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { ERC165_ID_ObjectFactory(call = defaultCall): Promise<[
                ...
                packageId: Buffer;
                active: boolean;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["F28FC5959B58C6DCD0FF491115678E72221B7AC69DB1275AF9265EDDEBCEC890"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogArchetypePackageActivation()); }, start, end); }, LogArchetypePackageCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                packageId: Buffer;
                author: string;
                isPrivate: boolean;
                active: boolean;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["F1C38AB7ABE967E483BB00E2556571B33037426B77DDB27A2704A68930053A9B"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogArchetypePackageCreation()); }, start, end); }, LogArchetypeToPackageUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                packageId: Buffer;
                archetypeAddress: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["C4121BE009B542B8D5F15B21E4B055CBF761E5B1FAC3ECEEBF4A330949480B9C"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogArchetypeToPackageUpdate()); }, start, end); } } as const, listenerFor: <T extends EventName>(eventNames: T[]): TypedListener<T> => listenerFor(client, address, events, decode, eventNames) as unknown as TypedListener<T>, listener: listenerFor(client, address, events, decode, Object.keys(events) as EventName[]) as TypedListener<EventName> } as const);
