...
contract Completables is CompletableOptions, AbstractVersionedArtifact(1, 2, 0), AbstractUpgradeable {
  ...
  // Contract will only emit events once active
  bool active;

  // Activate Completables to begin emitting events
  ...
  */
  ...
  /**
   * @notice attest to this this completable, deriving the franchisee from the caller via the agreement associated with
   * the completable - with the party being inferred from the actor.
  ...
  returns (bool)
  {
    ...
    ErrorsLib.revertIf(actor == address(0),
      ...
      Strings.concat("The caller is not authorized to attest to ", intervalId.toHex()));
  ...
  /**
   * @notice attest to this this completable, deriving the franchisee from the caller via the agreement associated with
   * the completable - attempting to attest as the specified party.
   * @return whether the Completable is ratified after this invocation
   */
  ...
  function executeAttest(bytes32 intervalId, int timestamp, address actor, address party)
  internal
  completableExists(intervalId)
  ...
  */
  ...
  // TODO: cannot pass this struct as argument without upgrading solidity to 0.8
  //  function push(Completable memory completable) public {
  //    if (active) {
  //      revert("Completables.push(): cannot push Completable after Completables are active");
  //    }
  //    completables[completable.intervalId] = completable;
  //    intervals.push(completable.intervalId);
  //  }
  //
  //  function migrateTo(address successor) public returns (bool success) {
  //    for (uint i = 0; i < intervals.length; i++) {
  //      Completables(successor).push(completables[intervals[i]]);
  //    }
  //
  //    return true;
  //  }
