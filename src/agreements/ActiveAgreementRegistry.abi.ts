...
export module ActiveAgreementRegistry {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { DATA_ID_AGREEMENT(call = defaultCall): Promise<[
                ...
                collectionType: number;
                packageId: Buffer;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["F15AA852A0CFD471FC73938CCB7ADEF244D3563B0D1A301ED863EEA9494E42E3"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogAgreementCollectionCreation()); }, start, end); }, LogAgreementExecutionProcessUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                agreementAddress: string;
                executionProcessInstance: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["A2394E9595D81855A19D93EDAED427A2284E2F0B79D0EFA57CA288536EF27C92"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogAgreementExecutionProcessUpdate()); }, start, end); }, LogAgreementFormationProcessUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                agreementAddress: string;
                formationProcessInstance: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["B9BF77BC620128CD4591B6007B72C8D403312DC11C4B8029E88140394110853E"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogAgreementFormationProcessUpdate()); }, start, end); }, LogAgreementToCollectionUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                collectionId: Buffer;
                agreementAddress: string;
            }) => CancelStreamSignal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): unknown { return client.listen(["8E2F60D6F5C049862987A5101D70110ACDB1C96F9B2D95F33F178C73990C642D"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.log.data, event?.log.topics).LogAgreementToCollectionUpdate()); }, start, end); } } as const, listenerFor: <T extends EventName>(eventNames: T[]): TypedListener<T> => listenerFor(client, address, events, decode, eventNames) as unknown as TypedListener<T>, listener: listenerFor(client, address, events, decode, Object.keys(events) as EventName[]) as TypedListener<EventName> } as const);
