...
export module ProcessInstance {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { EMPTY_SCOPE(call = defaultCall): Promise<[
                ...
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogEntityAddressScopeUpdate()); }, start, end); }, LogOwnerChanged(callback: (err?: Error, event?: {
                ...
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogOwnerChanged()); }, start, end); }, LogProcessInstanceCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                processInstanceAddress: string;
                processDefinitionAddress: string;
                state: number;
                startedBy: string;
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["3F4F1F4296FDBD3AFD2993CD0FA49B02B3B0AC165C0176C52BEEF3BB9FA6B56D"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogProcessInstanceCreation()); }, start, end); }, LogProcessInstanceStateUpdate(callback: (err?: Error, event?: {
                eventId: Buffer;
                processInstanceAddress: string;
                state: number;
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["BD6E1001C3E7D7E3C0E2AFE4E53839165BF847599263271503E06BFBD7DF6CAC"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogProcessInstanceStateUpdate()); }, start, end); } } as const, listenerFor: <T extends EventName>(eventNames: T[]): TypedListener<T> => listenerFor(client, address, events, decode, eventNames) as unknown as TypedListener<T>, listener: listenerFor(client, address, events, decode, Object.keys(events) as EventName[]) as TypedListener<EventName> } as const);
