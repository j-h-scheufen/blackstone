...
export module DefaultApplicationRegistry {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { DEFAULT_COMPLETION_FUNCTION(call = defaultCall): Promise<[
                ...
                dataType: number;
                direction: number;
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["E6CD28A5574A35135D11E8D4DBA593166A5980443E10BA70B4B753D4543FCAEC"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogApplicationAccessPointCreation()); }, start, end); }, LogApplicationCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                applicationId: Buffer;
                applicationType: number;
                location: string;
                method: Buffer;
                webForm: Buffer;
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["508C695AB328384FCDBBE37D4484A497A7AA922F6C0239F5FC5CE9D239D6565C"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogApplicationCreation()); }, start, end); }, LogUpgradeOwnerChanged(callback: (err?: Error, event?: {
