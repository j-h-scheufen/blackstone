...
export module DefaultArtifactsRegistry {
    ...
    export const contract = (client: Provider, address: string) => ({ address, functions: { EVENT_ID_ARTIFACTS(call = defaultCall): Promise<[
            ...
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["F8016D498A8282BE215351964850D99DDCAEC99A5D2E28843A65B7BAE6BF16D2"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogArtifactActivation()); }, start, end); }, LogArtifactCreation(callback: (err?: Error, event?: {
                eventId: Buffer;
                artifactId: string;
                artifactAddress: string;
                versionMajor: number;
                ...
                versionPatch: number;
                activeVersion: boolean;
            }) => Signal | void, start?: "first" | "latest" | "stream" | number, end?: "first" | "latest" | "stream" | number): EventStream { return client.listen(["2BCD5E595A4F50E426B161FAE964FB84844B5DC1EFC2B3799FD6D8924765120F"], address, (err?: Error, event?: Event) => { if (err)
                return callback(err); return callback(undefined, decode(client, event?.getLog()?.getData_asU8(), event?.getLog()?.getTopicsList_asU8()).LogArtifactCreation()); }, start, end); }, LogSystemOwnerChanged(callback: (err?: Error, event?: {
