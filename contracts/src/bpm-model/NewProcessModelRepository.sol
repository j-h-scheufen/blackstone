 ...
 */
contract NewProcessModelRepository is AbstractVersionedArtifact(1,1,0), AbstractObjectFactory, ArtifactsFinderEnabled, AbstractDbUpgradeable, ProcessModelRepository {
	...
	function verifyOldRepositoryDisabled(address _oldRepo) external returns (bool) {
		// this should not succeed due to the DB having changed system ownership
		// for testing purposes only!
		string memory someString = "bla";
		uint8[3] memory version = [3,5,1];
		return !_oldRepo.call(abi.encodeWithSignature("createProcessModel(bytes32,string,uint8[3],address,bool,string)",
					bytes32("modelId"), someString, version, address(this), false, someString));
	}
}